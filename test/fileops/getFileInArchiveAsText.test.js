...
const getFileInArchiveAsText = require('../../dist/fileops/getFileInArchiveAsText').default;
...
describe('fileops/getFileInArchiveAsText()', function(){
	let ipcRenderer;
	beforeEach(function(){
	...
	it('returns a Promise<string> when called', function(done){
		...
		let p;
		try {
			p = getFileInArchiveAsText('foo/bar', 'foo.zip');
